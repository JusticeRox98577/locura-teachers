...
//
// If your table name is different, change TBL_CLASSES here.
...
async function createClass({ teacher_name, level, period }) {
  setError("");
  ...
  if (!level) throw new Error("Class level is required.");
...
function renderCreateClass() {
  ...
  const level = el("input", { class: "input", id: "tLevel", placeholder: "Class level (e.g., Spanish 2)" });
  ...
  return el("div", { class: "card" }, [
    ...
    el("div", { class: "grid2" }, [
      ...
      el("div", {}, [el("div", { class: "label" }, ["Class level"]), level]),
...
function renderClassList() {
  ...
  for (const c of classes) {
    const title = `${c.teacher_name} • ${c.level} • Period ${c.period}`;
