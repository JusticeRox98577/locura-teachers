...
function leaderboardTable(rows) {
  ...
  return `
    ...
    <table class="adminTable">
      <thead>
        <tr><th>Points</th><th>Name</th><th>Class</th><th>User</th></tr>
      </thead>
      <tbody>${body}</tbody>
    </table>
  `;
...
async function renderTeacherDashboard(user, profile) {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      wrap.innerHTML = rows.length ? leaderboardTable(rows) : `<div class="smallMuted">No locked brackets yet.</div>`;
    } catch (e) {
...
async function main() {
  ...
  document.querySelector("#topOut").onclick = signOut;
